...
import utils_dns as utils


PROCESSES = 32 # multiprocessing.cpu_count()
MAXTRIES = 50
MAXFILELEN = 100
...
def build_audio(is_clean, params, filenum, audio_samples_length=-1):
    ...
    if is_clean and params['keep_clean']:
        with idx_counter.get_lock():
            idx = idx_counter.value % np.size(source_files)
            idx_counter.value += 1
        output_audio, fs_input = audioread(source_files[idx])
        if fs_input != fs_output:
            output_audio = librosa.resample(output_audio, fs_input, fs_output)
...
def main_gen(params, filenum):
    ...
    hyphen = '-'
    suffix_len = len(params['speech_suffix']) + 1
    # suffix_len = 5
    clean_source_filenamesonly = [i[:-suffix_len].split(os.path.sep)[-1] for i in clean_source_files]
    clean_files_joined = hyphen.join(clean_source_filenamesonly)[:MAXFILELEN]
    # noise_source_filenamesonly = [i[:-suffix_len].split(os.path.sep)[-1] for i in noise_source_files]
    # noise_files_joined = hyphen.join(noise_source_filenamesonly)[:MAXFILELEN]

    # noisyfilename = "{:07d}".format(filenum) + "_" + clean_files_joined + '_' + noise_files_joined + '_snr' + \
    #                 str(snr) + '.wav'
    # cleanfilename = "{:07d}".format(filenum) + "_" + clean_files_joined + '.wav'
    # noisefilename = "{:07d}".format(filenum) + "_" + clean_files_joined + '.wav'
    noisyfilename = cleanfilename = noisefilename = clean_files_joined + '.wav'
...
def main_body():
    ...
    if cfg.get('speech_dir') != 'None':
        clean_dir = cfg['speech_dir']
    if (not os.path.exists(clean_dir)) and ('speech_scp' not in cfg.keys() or cfg['speech_scp'] == 'None'):
    ...
    if (not os.path.exists(noise_dir)) and ('noise_scp' not in cfg.keys() or cfg['noise_scp'] == 'None'):
    ...
    params['keep_clean'] = False
    if 'keep_clean' in cfg.keys() and cfg['keep_clean'] != 'None':
        params['keep_clean'] = utils.str2bool(cfg['keep_clean'])
    ...
    elif 'speech_scp' in cfg.keys() and cfg['speech_scp'] != 'None':
        cleanfilenames = []
        with open(cfg['speech_scp'], "r") as f:
            for line in f.readlines():
                if len(line.split(" ")) == 1:
                    cleanfilenames.append(line.split(" ")[0].strip())
                else:
                    cleanfilenames.append(line.split(" ")[1].strip())
    else:
        cleanfilenames = glob.glob(os.path.join(clean_dir, params['audioformat']))
    
    params['speech_suffix'] = 'wav'
    if 'speech_suffix' in cfg.keys() and cfg['speech_suffix'] != 'None':
        params['speech_suffix'] = cfg['speech_suffix']
    params['cleanfilenames'] = cleanfilenames
    shuffle(params['cleanfilenames'])
    params['num_cleanfiles'] = len(params['cleanfilenames'])
    if params['keep_clean']:
        params['num_files'] = params['num_cleanfiles']
    if 'num_files' in cfg.keys() and cfg['num_files'] != 'None':
        params['num_files'] = int(cfg['num_files'])

    if 'noise_scp' in cfg.keys() and cfg['noise_scp'] != 'None':
        noisefilenames = []
        with open(cfg['noise_scp'], "r") as f:
            for line in f.readlines():
                if len(line.split(" ")) == 1:
                    noisefilenames.append(line.split(" ")[0].strip())
                else:
                    noisefilenames.append(line.split(" ")[1].strip())
